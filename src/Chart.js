...
const Branch = props => <line stroke="green" strokeWidth="0.2" {...props} />;
...
const Split = props => (
  <g>
    ...
    <Outcome x={props.x - 1 - 0.5} y={props.y + 2} textAnchor="end">
      {props.option[0]}
    </Outcome>
    <Outcome x={props.x + 1 + 0.4} y={props.y + 2} textAnchor="start">
      {props.option[1]}
    </Outcome>
  </g>
);
...
class Chart extends React.Component {
  ...
  render() {
    ...
    const dataNodes = this.state.data.map(function(item, i) {
      ...
      if (
        (pos === 4 && item.selectedOption === 1) ||
        (pos === -4 && item.selectedOption === 0)
      ) {
    ...
    const h = 256 + this.state.data.length * 42 + "px";
    return (
      <div id="chart-root">
        <svg width="100%" height={h}>
          ...
          <g>
            <g style={{ transform: "translate(50%, 42px) scale(20,20)" }}>
              <Outcome x="0" y="0">
                Start U-Splitter
              </Outcome>
              {dataNodes}
